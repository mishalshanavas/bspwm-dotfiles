#!/bin/bash
# Combined network info script - outputs icon and tooltip in one call  
# Usage: get_network_info.sh [icon|tooltip]

get_network_data() {
    if command -v nmcli >/dev/null 2>&1; then
        wifi_status=$(nmcli -t -f WIFI g)
        if [ "$wifi_status" = "enabled" ]; then
            active_connection=$(nmcli -t -f active,ssid,signal,freq,security dev wifi | grep '^yes')
            if [ -n "$active_connection" ]; then
                connected="true"
                ssid=$(echo "$active_connection" | cut -d: -f2)
                signal=$(echo "$active_connection" | cut -d: -f3)
                freq=$(echo "$active_connection" | cut -d: -f4)
                security=$(echo "$active_connection" | cut -d: -f5)
                ip_addr=$(ip route get 8.8.8.8 2>/dev/null | grep -oP 'src \K\S+' || echo "N/A")
            else
                connected="false"
                ssid=""; signal=""; freq=""; security=""; ip_addr=""
            fi
        else
            connected="false"
            wifi_enabled="false"
        fi
    elif command -v iwgetid >/dev/null 2>&1; then
        ssid=$(iwgetid -r 2>/dev/null)
        if [ -n "$ssid" ]; then
            connected="true"
            interface=$(iwconfig 2>/dev/null | grep -o '^[a-zA-Z0-9]*' | head -1)
            signal_info=$(iwconfig "$interface" 2>/dev/null | grep 'Signal level')
            ip_addr=$(ip route get 8.8.8.8 2>/dev/null | grep -oP 'src \K\S+' || echo "N/A")
        else
            connected="false"
        fi
    else
        connected="unknown"
        ip_addr=$(ip route get 8.8.8.8 2>/dev/null | grep -oP 'src \K\S+' || echo "N/A")
    fi
}

case "$1" in
    "icon")
        get_network_data
        if [ "$connected" = "true" ]; then
            echo "󰖩"  # Connected WiFi
        else
            echo "󰖪"  # Disconnected WiFi
        fi
        ;;
    "ssid")
        get_network_data
        if [ "$connected" = "true" ] && [ -n "$ssid" ]; then
            echo "$ssid"
        else
            echo "Disconnected"
        fi
        ;;
    "tooltip")
        get_network_data
        if [ "$connected" = "true" ] && [ -n "$ssid" ]; then
            if [ -n "$signal" ]; then
                echo "Network: $ssid
Signal: $signal%
Frequency: ${freq} MHz
Security: $security
IP: $ip_addr"
            else
                echo "Network: $ssid
$signal_info
IP: $ip_addr"
            fi
        elif [ "$wifi_enabled" = "false" ]; then
            echo "WiFi disabled"
        else
            echo "No WiFi connection"
        fi
        ;;
    *)
        echo "Usage: $0 [icon|ssid|tooltip]"
        ;;
esac